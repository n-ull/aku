    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if card.is_wild:
            ...
            await interaction.response.send_message(
                content="Select a color for your WILD, you have 60 seconds.",
                view=self.view.children_view,
                ephemeral=True,
            )
            await self.view.children_view.wait()

            # if not self.view.children_view.foo:  # if player didn't select a color...
...
class DrawButton(discord.ui.Button):
    async def callback(self, interaction: discord.Interaction):
        ...
        if interaction.data.get("custom_id") == "stack_resolve":
            self.view.game.last_action = (
                f"{self.view.game.current_player.name} draw {self.view.game.stack} cards, last card is: "
            )
            ...
            if card.validate(self.view.game.last_card) and not card.is_wild:
                ...
                await interaction.response.send_message(
                    content=f"Do you want to keep `[{card.name}]` {card.emoji} or play it?",
                    ephemeral=True,
                    view=self.children_view,
                )
...
class TurnOptionsView(discord.ui.View):
    ...
    async def close_view(self):
        ...
        if self.ephemeral_message is not None:
            self.ephemeral_message = None  # deletes the hand message webhook
        if self.children_view is not None:
            self.children_view.stop()
        ...
        if self.game.status.name == "PLAYING":
            ...
            new_view = await self.game.message_handler.create_new_menu(
                HandButton(label="Hand", custom_id="hand_button", style=color)
            )
    ...
    def build_turn_menu(self):
        ...
        if len(valid_hand) > 0:
            ...
            for card in valid_hand:
                card_selector.append_option(
                    discord.SelectOption(label=card.name, emoji=card.color_emoji, value=card.id)
                )
        ...
        if self.game.stack > 0 and self.game.data.stackable:
            self.add_item(
                DrawButton(
                    label=f"Draw {self.game.stack} cards", style=discord.ButtonStyle.danger, custom_id="stack_resolve"
                )
            )
        else:
            # add draw button
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # is the player turn
            ...
            if self.view.TURN_VIEW is not None:
                if self.view.TURN_VIEW.second_action:
                    return await interaction.response.send_message("You've already make your choice...", ephemeral=True)
            ...
            self.view.last_hand_msg = await interaction.followup.send(
                content=player_emoji_hand, ephemeral=True, view=self.view.TURN_VIEW
            )
        ...
        if self.game is not None:
            if self.game.status.name == "PLAYING":
                ...
                if self.TURN_VIEW is not None:
                    ...
                    self.TURN_VIEW.stop()
                    if self.TURN_VIEW.children_view is not None:
                        self.TURN_VIEW.children_view.stop()
...
class StartMenuView(discord.ui.View):
    ...
    async def start_button(self, interaction: discord.Interaction, button):
        if interaction.user.id == self.game.data.owner.id:
            ...
            FIRST_VIEW = await self.game.message_handler.create_new_menu(
                HandButton(label="Hand", custom_id="hand_button", style=discord.ButtonStyle.blurple)
            )
...
class GameDiscordInterface:
    ...
    GAME_STATUS_EMBED = (
        discord.Embed()
        .set_author(name="<< UNO >>")
        .add_field(name="Orientation", value="⤵ DOWN")
        .add_field(name="Stack", value="0")
    )
    ...
    async def send_status_message(self, view: discord.ui.View | None = None):
        ...
        self.GAME_STATUS_EMBED.set_field_at(
            index=0, name="Orientation: ", value=f"{'⏬ DOWN' if self.game.is_clockwise else '⏫ UP'}"
        )
        ...
        await self.thread.send(
            content=f"It's <@{self.game.current_player.id}>'s turn.", embed=self.GAME_STATUS_EMBED, view=view
        )
    ...
    async def start_message(self):
        ...
        self.GAME_START_EMBED.description = "Join UNO with us!." f"```md\n{self.game.player_list}```"
        self.GAME_START_EMBED.add_field(
            name="Rules:",
            value=f"""Stackable: {self.game.data.stackable}\nRandomize: {self.game.data.randomize_players}""",
        )
