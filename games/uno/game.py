...
class UNOGame(GameBase):
    ...
    async def add_player(self, user: discord.Member) -> str:
        # check if can add
        if self.status == GameState.PLAYING:
            return "Can't add a player while the game is running..."
        if user.id in [p.id for p in self.players]:
            return "You are already in the game..."
        if len(self.players) == self.data.max_players:
            return f"Maximum players: {self.data.max_players}"
    ...
    async def del_player(self, player: Player):
        if player not in self.players:
            return print("Player is not in this game...")
    ...
    def start_game(self):
        ...
        if len(self.players) < self.data.min_players:
            return f"Minimum of players is {self.data.min_players}"
        if self.data.randomize_players:
    ...
    async def punish_user(self):
        self.last_action = (
            ...
            if self.stack == 0
                ...
                f"{self.current_player.name} lose his turn and eat 3 cards plus {self.stack} "
                "stacked cards, last card is: "
            )
        )
        if self.current_player.warns == 2:
            ...
            if self.stack >= 1:
                await self.stack_resolve(force_skip=False)
            # logger.info(f"User punished: {self.current_player.name} has {self.current_player.warns} warns")
            self.skip_turn()
        new_view = await self.message_handler.create_new_menu(
