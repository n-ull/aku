...
import settings
from game_base import GameBase, GameState
...
class CardSelectView(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if the_actual_card.is_wild:
            ...
            await interaction.response.send_message(
                content="> Select a new color, you have 60 seconds.", view=wild_view, ephemeral=True
            )
            await wild_view.wait()

        await self.game.play_card(player, card_id)
        self.view.stop()
...
class DrawSelectView(GameView):
    ...
    async def on_timeout(self):
        self.game.last_action = (
            f"{self.game.players[self.game.current_player_index].name} picked up a card, the last card is: "
        )
        self.game.skip_turn()
...
class PlayingView(GameView):
    ...
    async def on_timeout(self):
        if self.hand_message is not None:
            await self.hand_message.edit(view=None)
        if self.card_select_view is not None:
            self.card_select_view.stop()
        if self.draw_select_view is not None:
            self.draw_select_view.stop()
    ...
    async def hand(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if player == self.game.players[self.game.current_player_index]:
            ...
            if len(valid_hand) == 0:
                ...
                self.hand_message = await interaction.followup.send(
                    content=f"{emoji_hand}", ephemeral=True, wait=True, view=self.card_select_view
                )
            ...
            player = self.game.get_player_by_id(interaction.user.id)
            await interaction.response.send_message(
                content=player.hand.emoji_hand(self.game.emoji_collection), ephemeral=True
            )
    ...
    async def draw(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if player == self.game.players[self.game.current_player_index]:
            ...
            if card.validate(self.game.graveyard.last_card) and not card.is_wild:
                ...
                self.draw_message = await interaction.followup.send(
                    content=(
                        f"> You picked up [`{card.name}`] {card.get_one_emoji(self.game.emoji_collection)},"
                        "do you want to throw it?, you got 60 seconds."
                    ),
                    ephemeral=True,
                    wait=True,
                    view=self.draw_select_view,
                )
                ...
                await interaction.response.send_message(
                    content=f"> You picked up [`{card.name}`] {card.get_one_emoji(self.game.emoji_collection)}",
                    ephemeral=True,
                )
                self.game.skip_turn()
            self.stop()
